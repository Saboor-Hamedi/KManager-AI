...
import CopyFigureButton from '../../../../../src/renderer/src/components/dashboard/CopyFigureButton'
...
describe('dashboard CopyFigureButton', () => {
  beforeEach(() => {
  ...
  it('renders copy button with default label', () => {
    ...
    expect(screen.getByTitle('Copy Figure to Clipboard')).toBeInTheDocument()
  })

  it('copies to clipboard on click and shows copied state', async () => {
    render(<CopyFigureButton targetRef={{ current: document.createElement('div') }} />)
    fireEvent.click(screen.getByTitle('Copy Figure to Clipboard'))
