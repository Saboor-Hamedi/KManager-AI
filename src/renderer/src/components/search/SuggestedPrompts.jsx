    ...
    <div className="select-none animate-in fade-in duration-300">
      <div className="flex flex-wrap gap-2">
        {prompts.map((promptText, idx) => (
          ...
          >
            <span className="shrink-0 text-[12px] opacity-70 group-hover:opacity-100 transition-opacity">💡</span>
            <span className="truncate">{promptText}</span>
            <ArrowRight size={11} className="shrink-0 opacity-50 group-hover:opacity-100 transition-opacity text-[var(--text-accent)]" />
          </button>
        ))}
      </div>
    </div>
