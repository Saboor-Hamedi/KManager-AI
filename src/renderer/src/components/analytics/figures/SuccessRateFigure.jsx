...
const COLORS = {
  'Successful (Hits > 0)': '#10b981', // green
  'Zero-Hit (No Results)': '#ef4444'  // red
}

const SuccessRateFigure = memo(({ data }) => {
  const chartRef = useRef(null)
  const chartData = data?.realCharts?.successBuckets || []
    ...
    <div ref={chartRef} className="bg-white/[0.01] border border-white/[0.04] rounded-[12px] p-4 hover:border-[var(--border-subtle)] transition-all flex-1 w-full min-w-0 relative">
      ...
      <div className="flex flex-col mb-2 pr-8">
        ...
        <p className="text-[11px] text-[var(--text-muted)] mt-0.5">
          Proportion of queries that found matching documents vs zero-hits
        </p>
      </div>
...
export default SuccessRateFigure
